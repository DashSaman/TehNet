...
- [ ] Add failing `tests/localbusiness-schema-contract.sh` first.
- [ ] Contract requires centralized business name/locality/region/country/on-site/remote settings and a schema renderer loaded by TehNet Core.
- [ ] Contract rejects deprecated `serviceArea` and invented rating/geo/opening-hours fields.
- [ ] Confirm RED.
- [ ] Refactor settings defaults into one reusable source while preserving existing shortcodes and sanitization.
- [ ] Add truthful defaults for business identity/service areas and the known YouTube identity URL.
- [ ] Re-run core/settings contracts and new contract to GREEN; commit.

### Task 2: LocalBusiness JSON-LD renderer
- [ ] Implement `class-schema.php` with conditional page targeting, PostalAddress, normalized telephone, stable `@id`, URL, `sameAs`, and `areaServed`.
- [ ] Derive streetAddress from the existing full display address without duplicating a second street-address source of truth.
- [ ] Output with `wp_json_encode` and unescaped Unicode/slashes.
- [ ] Keep output absent when required identity/address fields are incomplete.
- [ ] Run PHP lint and full repository suite; commit.

### Task 3: Safe production deployment and live validation
- [ ] Create a fresh TehNet DB/wp-content recovery point and fingerprint non-TehNet runtime.
- [ ] Deploy only TehNet theme/plugin files through the existing guarded deploy script; no container/Nginx restart.
- [ ] Validate JSON-LD syntax and exact values on homepage, Contact, About, Services hub and all five service owners.
- [ ] Verify no LocalBusiness script on `/learn/mikrotik/`.
- [ ] Verify `blog_public=0`, rendered `noindex,nofollow`, port `127.0.0.1:18082`, Nginx config and unrelated runtime unchanged.

### Task 4: Evidence, coordination and integration
- [ ] Record live evidence and recovery path in `ops/`.
- [ ] Update `PROGRESS.md`, `HANDOFF.md`, `TASKS.md` with verified facts only.
- [ ] Run all contracts, shell syntax/PHP lint, live JSON parse and secret-value diff scan.
